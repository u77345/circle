...
:	m_Screen (m_Options.GetWidth (), m_Options.GetHeight ()),
	...
	m_pSound(0),
	m_VFO(0)
	// NOT DOING GG // m_VFO (&m_LFO)		// LFO modulates the VFO
{
	//m_pSound = new CI2SSoundBaseDevice (&m_Interrupt, SAMPLE_RATE,
	//						  CHUNK_SIZE, false,
	//						  &m_I2CMaster, 0);

	m_ActLED.Blink (5);	// show we are alive
	CLogger::Get ()->Write (FromKernel, LogNotice, "ctor done");
}
...
TShutdownMode CKernel::Run (void)
{
	...
	//m_VFO.SetWaveform (WaveformTriangle);
	m_VFO.SetFrequency (1000.0);
	m_VFO.SetModulationVolume (0.00);
	...
	m_Logger.Write (FromKernel, LogNotice, "Playing 1KHz tone");
